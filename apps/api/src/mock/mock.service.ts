...
export class MockService {
  ...
  private sampleImages: SampleImageCollection | null = null;
  ...
  private async generateRandomInspection(): Promise<void> {
    ...
    // Pick a random batch
    const batch =
      activeBatches[Math.floor(Math.random() * activeBatches.length)];
    ...
    if (garments.length === 0) {
      ...
      if (batch.status === BatchStatus.IN_PROGRESS) {
        await this.batchService.updateBatchStatus(
          batch.id,
        ...
        await this.eventService.createEvent(
          batch.tenantId,
          EventType.BATCH_AWAITING_APPROVAL,
          {
            batchId: batch.id,
    ...
    if (hasDefect) {
      ...
      const weights = [0.15, 0.35, 0.15, 0.25, 0.1]; // stain, stitching, misprint, measurement, other
    ...
    try {
      photoKeyBefore = await this.uploadSampleImage(
        batch.tenantId,
        hasDefect,
        defectType,
      );
      if (hasDefect) {
        photoKeyAfter = await this.uploadSampleImage(batch.tenantId, false);
      }
    } catch (error) {
    ...
    const inspection = await this.inspectionService.createInspection({
      ...
      defectType,
      notes: hasDefect
        ? `${defectType} detected during quality inspection`
        : undefined,
      photoKeyBefore,
      photoKeyAfter,
  ...
  private ensureSampleImagesLoaded(): SampleImageCollection {
    if (!this.sampleImages) {
      this.sampleImages = loadSampleImages();

      if (
        this.sampleImages.normal.length === 0 &&
        this.sampleImages.defect.length === 0
      ) {
        console.warn(
          "[MockService] No seed images found. Falling back to generated placeholders.",
        );
      }
    }
  ...
  private async uploadSampleImage(
    ...
    // Fallback to generated placeholder if no images are available
