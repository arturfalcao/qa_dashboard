...
import {
  ...
  CreatePhotoAnnotationDto,
  UpdatePhotoAnnotationDto,
} from "../services/photo-annotation.service";
import { z } from "zod";
...
export class PhotoAnnotationController {
  ...
  async createAnnotation(
    ...
    createDto: z.infer<typeof CreateAnnotationSchema>,
    @Request() req: any,
  ) {
    ...
    return await this.photoAnnotationService.createAnnotation(
      finalTenantId,
      annotationDto,
    );
  }
  ...
  async getAnnotationsForPhoto(
    ...
    const finalTenantId = tenantId || "045f1210-98cc-457e-9d44-982a1875527d";
    return await this.photoAnnotationService.getAnnotationsForPhoto(
      finalTenantId,
      photoId,
    );
  }
  ...
  async getAnnotationById(
    ...
    const finalTenantId = tenantId || "045f1210-98cc-457e-9d44-982a1875527d";
    const annotation = await this.photoAnnotationService.getAnnotationById(
      finalTenantId,
      annotationId,
    );
    if (!annotation) {
  ...
  async updateAnnotation(
    ...
    updateDto: z.infer<typeof UpdateAnnotationSchema>,
    @Request() req: any,
  ) {
    const finalTenantId = tenantId || "045f1210-98cc-457e-9d44-982a1875527d";
  ...
  async deleteAnnotation(
    ...
    const finalTenantId = tenantId || "045f1210-98cc-457e-9d44-982a1875527d";
    await this.photoAnnotationService.deleteAnnotation(
      finalTenantId,
      annotationId,
      req.user?.id || "demo-user",
    );
