...
import { DefectType, DefectSeverity, ProcessStation } from '@qa-dashboard/shared';
...
export class Inspection {
  ...
  @Column({ name: 'humidity', type: 'decimal', precision: 4, scale: 1, nullable: true })
  humidity?: number;

  @Column({ name: 'lighting_level', type: 'decimal', precision: 6, scale: 1, nullable: true })
  lightingLevel?: number;

  @Column({ name: 'quality_score', type: 'int', nullable: true })
  qualityScore?: number;

  @Column({ name: 'inspected_at', type: 'timestamptz', default: () => 'CURRENT_TIMESTAMP' })
  inspectedAt: Date;
  ...
  @ManyToOne(() => Tenant, tenant => tenant.inspections, { onDelete: 'CASCADE' })
  @JoinColumn({ name: 'tenant_id' })
  tenant: Tenant;

  @ManyToOne(() => Garment, garment => garment.inspections, { onDelete: 'CASCADE' })
