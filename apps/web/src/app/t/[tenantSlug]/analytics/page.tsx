...
export default function AnalyticsPage() {
  ...
  const [groupBy, setGroupBy] = useState<'style' | 'vendor'>('vendor')
  ...
  const overallDefectRate = defectRate?.data?.find(d => d.name === 'Overall') || defectRate?.data?.[0]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard
          title="Defect Rate"
          value={`${overallDefectRate?.defectRate.toFixed(1) || '0.0'}%`}
          isLoading={defectRateLoading}
          trend={overallDefectRate?.defectRate && overallDefectRate.defectRate < 5 ? 'good' : 'warning'}
          icon="alert-triangle"
        />
        ...
        <MetricCard
          ...
          value={`${overallDefectRate?.totalInspected || 0}`}
          isLoading={defectRateLoading}
          subtitle={`${overallDefectRate?.totalDefects || 0} defects found`}
          icon="package"
        />
      </div>
