...
export default function ExportsPage() {
  ...
  const pdfMutation = useMutation({
    mutationFn: ({ batchId, range }: { batchId?: string; range?: 'last_7d' | 'last_30d' }) =>
      apiClient.generatePDF(batchId, range),
  })

  const csvMutation = useMutation({
    mutationFn: (range: 'last_7d' | 'last_30d') =>
      apiClient.generateCSV(range),
  })
              ...
              >
                ...
                {batches.map((batch: any) => (
                  <option key={batch.id} value={batch.id}>
                    {batch.poNumber} - {batch.vendor?.name}
                  </option>
                ))}
              </select>
            </div>
